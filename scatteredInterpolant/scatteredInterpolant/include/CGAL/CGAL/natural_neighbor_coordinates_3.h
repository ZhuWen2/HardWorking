				      ...
				      const typename Dt::Cell_handle start = CGAL_TYPENAME_DEFAULT_ARG Dt::Cell_handle())
{
  ...
  // Find the cells in conflict with Q
  //找到与Q冲突的四面体，即与Q点有交集的四面体并把他们的索引存放在cells中
  dt.find_conflicts(Q, c, 
		    Emptyset_iterator(),
		    std::inserter(cells,cells.begin()));
	...
	{//for each neighboring cell cc2 of cc1
	  ...
	  if(std::find(cells.begin(),cells.end(),cc2)==cells.end())
		...
		{//for each vertex P of the boundary facet
		  ...
		  Cell_handle next=cc3->neighbor(num_next);
		  //循环，如果next在cells中则进入循环，这个作用应该是让cc3保持在cells外，一旦在cells中就把它next
		  while (std::find(cells.begin(),cells.end(),next)!=cells.end())
		    { //next is in conflict
		      CGAL_triangulation_assertion( next != cc1 );
		      cc3=next;
...
//template <class Dt>
//void
//	Fast_ScatteredInterpolant(Dt& triangulation, Point3& pp, double& result, double& norm, std::vector<double> &weight, std::map<Point3, double>& value_f,
//	const typename Dt::Cell_handle start = CGAL_TYPENAME_DEFAULT_ARG Dt::Cell_handle())
//{
//	typedef typename Dt::Geom_traits Gt;
//	typedef typename Gt::Point_3 Point;
...
//	Locate_type lt; int li, lj;
//
//	Cell_handle c = triangulation.locate(pp, lt, li, lj, start);
//	for (int i = 0; i < 4; ++i) {
//		Vertex_handle vP = c->vertex(i);
//		Point3 p = vP->point();
//		double distance = sqrt(pow((pp.x() - p.x()), 2) + pow((pp.y() - p.y()), 2) + pow((pp.z() - p.z()), 2));
//		weight.push_back(1 / distance);
//
//		result += weight[i] * value_f[p];
//		norm += weight[i];
//	}
//	
//}
...
template <class Dt>
inline void
Fast_ScatteredInterpolant(Dt& triangulation, Point3& pp,
const typename Dt::Cell_handle start = CGAL_TYPENAME_DEFAULT_ARG Dt::Cell_handle())
{
	typedef typename Dt::Geom_traits Gt;
	typedef typename Gt::Point_3 Point;
	...
	for (int i = 0; i < 4; ++i) {
		Vertex_handle vP = c->vertex(i);
		Point3 p = vP->point();
		double weight_d = 1.0 / sqrtf(pow((pp.x() - p.x()), 2) + pow((pp.y() - p.y()), 2) + pow((pp.z() - p.z()), 2));
		pp.value += weight_d * p.value;
		norm += weight_d;
	}
	pp.value /= norm;
}
