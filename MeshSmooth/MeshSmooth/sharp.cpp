...
//#pragma omp parallel for
		...
		clock_t end2 = clock();
		std::cout << "time = " << double(end2 - start2) / CLOCKS_PER_SEC << "s" << std::endl;  //输出时间（单位：ｓ）
		//if (sharp_face.size() > max_sharp_num) {
		//	max_sharp_num = sharp_face.size();

		//}
		//else if (double(sharp_face.size()) / max_sharp_num < 0.8) {
		//	std::cout << std::endl;
		//	sharp_face.clear();
...
void GELSmooth::SearchFeatures(myMesh & mesh, myMesh::FaceHandle fh, int &count_s, std::vector<std::vector<const int> >& sharp_face_neiidx)
{
	...
	for (int i = 0; i < nei_normal.size() - 1; ++i) {
		...
		if ((double(count1) / hhhh >= 0.01) && (double(count2) / hhhh <=1)) {
