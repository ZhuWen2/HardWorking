...
void save_corr(boost::shared_ptr<pcl::Correspondences> cru_correspondences, pcl::PointCloud<pcl::PointXYZ>::Ptr source, pcl::PointCloud<pcl::PointXYZ>::Ptr target, int a, int b) {
    string filename1 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/fpfh_test/PT" + to_string(a) + "_" + to_string(b) + "_" + to_string(a) + ".txt";
    string filename2 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/fpfh_test/PT" + to_string(a) + "_" + to_string(b) + "_" + to_string(b) + ".txt";
...
void find_fpfh(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud1, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud2,int a,int b) {
    ...
    vs.setLeafSize(0.001, 0.001, 0.001);
    ...
    vt.setLeafSize(0.001, 0.001, 0.001);
...
double calaPointCloudCoincide(int m, int n, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_src, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_target, float para1)
{
	...
	for (int i = 0; i < (*cor).size(); ++i) {
		point.x = cloud_src->points[(*cor)[i].index_query].x;
		point.y = cloud_src->points[(*cor)[i].index_query].y;
		point.z = cloud_src->points[(*cor)[i].index_query].z;
		cloud1->push_back(point);

        
		point.x = cloud_target->points[(*cor)[i].index_match].x;
        if (match.count(point.x)>0) {
            ++num_re;
            continue;

        }
		point.y = cloud_target->points[(*cor)[i].index_match].y;
		point.z = cloud_target->points[(*cor)[i].index_match].z;
		cloud2->push_back(point);
        match.insert(point.x);
	}
    cout << "筛掉了" << num_re << "个重复点" << endl;
	pcl::io::savePLYFile("E:\\XTOP\\XTOPwork\\ICP\\测试数据\\test\\07\\overlap_cloud\\P" + to_string(m) +"_"+ to_string(n) + "_" + to_string(m)+".ply", *cloud1);
	pcl::io::savePLYFile("E:\\XTOP\\XTOPwork\\ICP\\测试数据\\test\\07\\overlap_cloud\\P" + to_string(m) + "_" + to_string(n) + "_" + to_string(n) + ".ply", *cloud2);
	//用fpfh找对应点
    clock_t e3 = clock();
    whole_time += e3 - s3;

	find_fpfh(cloud1, cloud2,m,n);
    cout << chonghe << endl;
	return chonghe;
}
...
void find_corr() {
	...
	for (int i = 0; i < 9; ++i) {
		...
		string fileName1 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/fpfh_test/PT" + to_string(i) + ".ply";
		...
		for (int j = i + 1; j < 10; ++j) {
			...
			string fileName2 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/fpfh_test/PT" + to_string(j) + ".ply";
			pcl::io::loadPLYFile(fileName2, *target);
			cout << "从目标点云中读取 " << target->size() << " 个点" << endl;
            clock_t e2 = clock();
            whole_time += e2 - s2;
			calaPointCloudCoincide(i, j, source, target, 1);
