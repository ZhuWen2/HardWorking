...
double calaPointCloudCoincide(int m, int n, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_src, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_target, float para1)
{
	...
	//打印出对应点
	std::string fileName1 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/P" + std::to_string(m) + "_" + std::to_string(n) + "_" + std::to_string(m) + ".txt";
	std::string fileName2 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/P" + std::to_string(m) + "_" + std::to_string(n) + "_" + std::to_string(n) + ".txt";
...
void find_corr() {
	for (int i = 0; i < 9; ++i) {
		...
		std::string fileName1 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/PT" + std::to_string(i) + ".ply";
		pcl::io::loadPLYFile(fileName1, *source);
		std::cout << "从源点云中读取 " << source->size() << " 个点" << std::endl;
		for (int j = i + 1; j < 10; ++j) {
			// -------------------加载目标点云----------------------
			pcl::PointCloud<pcl::PointXYZ>::Ptr target(new pcl::PointCloud<pcl::PointXYZ>);
			std::string fileName2 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/PT" + std::to_string(j) + ".ply";
...
int read(std::string filename_global_points, std::vector<std::string>& filenames) {
	...
	for (int i = 0; i < pointSum - 1; i++)
	{
		for (int j = i + 1; j < pointSum; ++j) {
			std::string file1 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/P" + std::to_string(i) + "_" + std::to_string(j) + "_" + std::to_string(i) + ".txt";
			std::string file2 = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/P" + std::to_string(i) + "_" + std::to_string(j) + "_" + std::to_string(j) + ".txt";
...
{
	clock_t start = clock();
	for (int itnum = 0; itnum < 3; ++itnum) {
		std::cout.precision(12);

		std::string folder = "E:/XTOP/XTOPwork/optimation/optimation - 副本/test/PT";
		...
		for (int i = 0; i < Length_Names; i++)
		{
			...
			fileName = fileName.substr(52, fileName.size());
		...
		options0.max_num_iterations = 200;
		options0.parameter_tolerance = 1e-8;
